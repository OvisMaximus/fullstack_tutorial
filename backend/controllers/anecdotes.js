...
const getAllAnecdotes = async (request, response) => {
    ...
    const anecdotes = await Anecdote.find({}).populate('user', { username: 1, name: 1 })
...
const addAnecdote = async (request, response) => {
    ...
    const savedAnecdote = await Anecdote(await anecdote.save())
...
const updateAnecdote = async (request, response) => {
    ...
    const populatedAnecdote = await Anecdote(savedAnecdote).populate('user', { username: 1, name: 1 })
