...
describe('when there is initially some notes saved', () => {
    ...
    describe('addition of a new note', () => {
        test('succeeds with valid data', async () => {
            const userToken = await userHelper.authenticatedUserToken(
                userHelper.initialUsers[1],
                api,
            )
        ...
        test('after adding a new note it is listed in the notes list of a user', async () => {
            ...
            await api
                ...
                .expect(201)
            const notesAtEnd = await helper.notesInDb()
            const storedNote = notesAtEnd.filter((note) =>
                note.content.startsWith('population'),
            )[0]
            ...
            const matchingNotesInUser = noteDbUser.notes.filter(
                (note) => note.id === storedNote.id,
            )
    ...
    describe('deletion of a note', () => {
        test('succeeds with status code 204 if the id is valid', async () => {
            ...
            const user = userHelper.initialUsers.filter(
                (user) => user.username === noteToDelete.user.username,
            )[0]
        ...
        test('fails with status code 403 (Forbidden) if the user is not the creator', async () => {
            const notesAtStart = await helper.notesInDb()
            const noteToDelete = notesAtStart[0]
            const token = await userHelper.tokenOfDifferentUser(
                noteToDelete.user,
                api,
            )
    ...
    describe('modification of a note', () => {
        test('succeeds with status code 200 if data is valid and user is authenticated', async () => {
            ...
            const user = userHelper.initialUsers.filter(
                (user) => user.username === noteToModify.user.username,
            )[0]
            const token = await userHelper.authenticatedUserToken(user, api)
