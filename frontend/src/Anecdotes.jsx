import { useState } from 'react'

const Header = ({headerText}) => (<h1>{headerText}</h1>)
const Button = ({onClick, text}) => <button onClick={onClick}>{text}</button>

const Votes = ({value}) => <div>has {value} votes</div>

const App = () => {
    const anecdotes = [
        'If it hurts, do it more often.',
        'Adding manpower to a late software project makes it later!',
        'The first 90 percent of the code accounts for the first 90 percent of the development time...The remaining 10 percent of the code accounts for the other 90 percent of the development time.',
        'Any fool can write code that a computer can understand. Good programmers write code that humans can understand.',
        'Premature optimization is the root of all evil.',
        'Debugging is twice as hard as writing the code in the first place. Therefore, if you write the code as cleverly as possible, you are, by definition, not smart enough to debug it.',
        'Programming without an extremely heavy use of console.log is same as if a doctor would refuse to use x-rays or blood tests when diagnosing patients.',
        'The only way to go fast, is to go well.'
    ]

    const getRandomIndex = (array) => Math.floor(Math.random() * array.length)
    const [selected, setSelected] = useState(getRandomIndex(anecdotes))
    const [votesList, setVotesList] = useState(Array(anecdotes.length).fill(0))
    const nextRandomAnecdote = () => setSelected(getRandomIndex(anecdotes))
    const upvote = (index) => () => {
        const newVotesList = [...votesList]
        newVotesList[index] += 1
        setVotesList(newVotesList)
    }
    const indexWithMostVotes = votesList.indexOf(Math.max(...votesList))

    return (
        <div>
            <Header headerText='Anecdote of the day'/>
            {anecdotes[selected]}<br/>
            <Votes value={votesList[selected]}/>
            <Button onClick={upvote(selected)} text='vote'/>
            <Button onClick={nextRandomAnecdote} text='next anecdote'/>
            <Header headerText='Anecdote with most votes'/>
            {anecdotes[indexWithMostVotes]}<br/>
            <Votes value={votesList[indexWithMostVotes]}/>
        </div>
    )
}

export default App